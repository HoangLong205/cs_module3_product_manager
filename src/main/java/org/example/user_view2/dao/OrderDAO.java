package org.example.user_view2.dao;

import org.example.user_view2.models.*;
import org.example.user_view2.utils.DBConnection;
        ...
        try (Connection cn = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    it.setUnitPrice(rs.getDouble("price"));
                    items.add(it);
                }
            }
        } catch (SQLException e) { throw new RuntimeException(e); }
